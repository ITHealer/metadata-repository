...
**Branch:** `docs/pr-04-metadata-guidelines`
**Title:** `docs(metadata): define reviewer and LLM transformation guidelines`
...
- [ ] Tạo `metadata/review/_template.md`.
...
## 8. PR-05 — Domain models, draft generator và validation core

**Depends on:** PR-03, PR-04
**Branch:** `feat/pr-05-metadata-validation-core`
**Title:** `feat(metadata): add review models, draft generation, and validation`

### Mục tiêu

Tạo lõi deterministic để sinh review draft, parse front matter, tính schema hash và validate metadata.

### Tasks

- [ ] Implement immutable/validated domain models:
  - `ReviewDocument`
  - `TableSchema`
  - `ColumnSchema`
  - `Relation`
  - `Evidence`
- [ ] Implement enums cho status và evidence state.
- [ ] Implement front matter reader/writer có stable ordering.
- [ ] Implement canonical schema hash; thêm test chống hash thay đổi vì key order.
- [ ] Implement `CreateDrafts` use case:
  - Tạo draft cho table mới.
  - Không ghi đè reviewer content hiện có.
  - Schema đổi thì cập nhật hash và hạ status về `needs_review`.
- [ ] Implement review validator:
  - Required fields/headings.
  - Table/column/relation tồn tại.
  - Conditional rule cho timestamp, measure, status và PII.
  - Evidence bắt buộc khi `approved`.
  - Guideline version hiện hành.
- [ ] CLI commands:
  - `metadata draft`
  - `metadata validate-review`
- [ ] Unit tests cho happy path và từng validation error.
- [ ] Snapshot/golden tests cho ba draft demo.
...
- Chạy draft hai lần không tạo diff khi schema không đổi.
- Nội dung reviewer không bị ghi đè.
- Column giả, stale schema hash và approved thiếu evidence đều fail đúng error code.
