...
├── ports/
│   ├── document_generator.py # Protocol structured output, không phụ thuộc provider
...
- [ ] Tạo `config/databases/commerce_demo/tbls.yml` với DSN qua environment variable.
...
- [ ] Tạo `contracts/metadata_contract.yml` làm single source cho:
  - Guideline versions.
  - Required headings.
...
## 9. PR-06 — Deterministic publish và semantic chunk contract

**Depends on:** PR-05
**Branch:** `feat/pr-06-publish-and-chunk`
**Title:** `feat(knowledge): publish metadata and build semantic chunks`

### Mục tiêu

Hợp nhất technical facts từ raw `schema.json` với business facts từ reviewer YAML thành published
Markdown và semantic chunks có cấu trúc, deterministic và trace được về nguồn. PR này xây baseline
không cần network/secret; live LLM ở PR-10 chỉ được thay đổi cách diễn đạt và phải trả về cùng output
contract.
...
Publish phải chạy review validation trước khi tạo output. Validation error, stale schema hash,
unknown technical reference hoặc evidence `conflicting` đều chặn publish. Warning của tài liệu
`needs_review` không chặn preview.
...
Published front matter bắt buộc giữ: `database`, `table`, `document_status`, `schema_hash`, ba contract/
guideline versions, `source_schema_path`, `source_review_path`, `source_review_commit` và
`generator_mode`. Body render đúng thứ tự section trong Guideline 2: Summary; Grain and purpose;
Appropriate use; Inappropriate use; Columns; Relationships and join risks; Business rules; Time and
unit semantics; Data quality and caveats; Security; Evidence.

`needs_review` vẫn được render và gắn nhãn preview rõ ràng; chunk tương ứng phải có
`index_eligible: false`. Chunk của `approved` có `index_eligible: true`; PR-06 chưa thực hiện indexing.
...
- [ ] Định nghĩa strict domain models/enums:
  - `PublishedDocument`, `PublishedColumn`, `PublishedRelationship`, `PublishedRule`, `Provenance`.
  - `Chunk`, `ChunkType`, `EvidenceReference`.
...
- [ ] Implement `PublishMetadata` use case:
  ...
  - Reuse review validation; promote `conflicting_evidence` thành blocking publish error kể cả khi
    document còn `needs_review`.
  - Áp dụng source precedence theo từng fact type, không dùng một global dictionary overwrite.
  - Copy exact technical type/nullability/comment từ raw schema.
  - Preserve reviewer uncertainty, caveat và evidence status; không nâng claim/status.
  - Nhận `source_review_commit` qua input và tạo provenance đầy đủ.
  - Không ghi một phần output nếu bất kỳ document nào fail preflight.
- [ ] Implement deterministic `PublishedMarkdownRenderer` và atomic writer:
  - Stable front matter ordering, section ordering, UTF-8 và final newline.
  - Heading không áp dụng ghi `Not applicable` cùng lý do, không để section rỗng.
  - Chạy hai lần cùng input không rewrite file/không tạo Git diff.
  - Xóa file published orphan chỉ khi raw/reviewer preflight đã pass và trả action rõ ràng.
- [ ] Implement `BuildChunks` trực tiếp từ structured `PublishedDocument` với đúng Guideline 2 types:
  - `table_overview`.
  - `column_group`.
  - `relationship`.
  - `business_rule`.
  - `quality_and_security`.
- [ ] Implement stable inspectable IDs:
  - Parent document reference: `<database>.<table>::document`; đây là identity của
    `PublishedDocument`, không phải một chunk type mới.
  - Child: `<database>.<table>::<chunk_type>::<semantic_key>`.
  - Content thay đổi không đổi identity của cùng semantic unit; schema/review versions nằm trong
    metadata để PR-09 thay đúng version.
- [ ] Implement deterministic JSONL writer cho chunk dry-run:
  - Stable chunk order và JSON key order.
  - Atomic output; byte-identical khi input không đổi.
  - Không ghi credentials, DSN hoặc row data.
- [ ] Implement published/chunk validators:
  - Provenance path/hash/version/commit khớp input.
  - Published technical facts và identifiers khớp raw schema.
  - Không section/chunk rỗng; chunk IDs unique và `parent_document_id` khớp published document.
  - `table_overview` giữ purpose + grain + use/not-use + major caveat.
  - `column_group` giữ exact column + meaning + unit/time/value semantics + evidence.
  - `relationship` giữ cả hai endpoint, join condition, cardinality, row-count risk + evidence.
  - `business_rule` giữ rule + scope/exception + evidence.
  - `quality_and_security` giữ issue/classification + affected context + safe-use instruction.
  - `needs_review` không bao giờ có `index_eligible: true`.
- [ ] CLI commands:
  - `metadata publish --mode mock --source-review-commit <sha>`.
  - `metadata validate-published`.
  - `metadata chunk --dry-run --output build/chunks/<database>.jsonl`.
- [ ] Make targets:
  - `make publish SOURCE_REVIEW_COMMIT=<sha>`.
  - `make published-validate`.
  - `make chunk-dry-run`.
  - `make knowledge-check` chạy publish → validate → chunk dry-run và fail nếu committed published
    output bị stale.
- [ ] Tests:
  - Unit tests cho source precedence, conflict, provenance, stable ID và từng co-location rule.
  - Contract test render cả ba committed `needs_review` documents với `index_eligible: false`.
  - Approved `orders` fixture chứng minh `index_eligible: true` nhưng chưa index.
  - Golden Markdown + JSONL cho `orders`.
  - Idempotency/golden test bảo đảm byte-identical và không rewrite khi input/version/commit không đổi.
  - Failure test bảo đảm preflight lỗi không để lại partial output.

### Clean-code constraints

- Domain models không import filesystem, Git, Markdown parser hoặc ClickHouse/tbls adapter.
- Application use cases nhận DTO/model và trả result/issue; không `print` và không tự chạy Git.
- Renderer/writer nằm trong `io`; source loading nằm trong adapter hiện có.
- Chunker dùng structured model, không scrape Markdown headings vừa render.
- Validator trả actionable issue với `code`, `path`, `field`, `message`, `severity`.
- Published/chunk output không trở thành nguồn để sửa ngược reviewer metadata.

### Acceptance criteria

- Ba reviewer documents hiện tại sinh preview Markdown đúng source precedence; chunks đều
  `index_eligible: false` vì documents còn `needs_review`.
- Approved `orders` fixture sinh đủ provenance/required sections và chunks
  `index_eligible: true`.
- Mỗi chunk có metadata bắt buộc, stable ID, parent reference và self-contained context.
- Relationship chunk chứa đủ hai table, join condition, cardinality, row-count/duplicate risk và
  evidence trong cùng chunk.
- Measure/timestamp/status context không bị tách khỏi unit/time/value semantics.
- Evidence `conflicting`, stale hash, unknown identifier hoặc published technical drift fail với error
  code cụ thể và không để lại partial output.
- Chạy publish/chunk hai lần với cùng inputs + source commit tạo byte-identical output và không Git
  diff.
- `make verify` và `make knowledge-check SOURCE_REVIEW_COMMIT=<fixed-sha>` pass không cần network,
  token hoặc secret.
