...
3. Pipeline tạo bản metadata nghiệp vụ ban đầu trong `metadata/review/`.
4. Domain reviewer sửa file review và commit lên branch của Pull Request (PR).
5. GitHub Actions nhận commit mới, sinh lại tài liệu chuẩn hóa trong `knowledge/published/` và bot commit vào cùng branch.
...
MVP giữ GitHub repository làm source of truth, PR làm trung tâm review và commit vào `metadata/review/**` là feedback chính thức.
...
|---|---|---|
| `schema/raw/` | Chỉ pipeline `tbls` | Schema kỹ thuật lấy từ database |
| `metadata/review/` | AI tạo draft, reviewer chỉnh sửa | Nguồn metadata nghiệp vụ có thể audit |
| `knowledge/published/` | Chỉ pipeline generate | Tài liệu chuẩn hóa cuối cùng để index |

Nguyên tắc bắt buộc:

- Reviewer không sửa `schema/raw/**` hoặc `knowledge/published/**`.
- AI không suy feedback từ file do `tbls` sinh; AI đọc `schema.json`, file review, prompt và guideline.
- Chỉ commit thay đổi `schema/raw/**` hoặc `metadata/review/**` mới yêu cầu generate lại.
...
| Domain Reviewer | Sửa `metadata/review/**`, xác nhận purpose, grain, rule, caveat |
| GitHub Bot | Commit `schema/raw/**` và `knowledge/published/**` vào source branch của PR |
...
```mermaid
flowchart LR
    A["ClickHouse Docker<br/>commerce_demo"] -->|"tbls doc"| B["schema/raw/commerce_demo<br/>Markdown + schema.json"]
    B --> C["Draft generator"]
    C --> D["metadata/review/<br/>Human-editable"]
    D -->|"Reviewer commit"| E["GitHub Pull Request"]
    E -->|"GitHub Actions"| F["Normalize + validate"]
    F -->|"Bot commit"| G["knowledge/published/<br/>Generated only"]
    G --> E
...
services:
  ...
  tbls:
    ...
    environment:
      ...
      TBLS_DOC_PATH: schema/raw/commerce_demo
    depends_on:
      clickhouse:
...
dsn: ${TBLS_DSN}
docPath: schema/raw/commerce_demo
...
- `document_status: approved` chỉ do reviewer thay đổi trong `metadata/review/**`.
...
Guideline này phải được liên kết trong PR template và ở đầu mỗi draft review. Reviewer không cần sửa file do `tbls` sinh; toàn bộ thông tin nghiệp vụ được bổ sung trong `metadata/review/**`.
...
2. `metadata/review/**` là sự thật nghiệp vụ nếu claim đã có reviewer/evidence.
...
Mỗi file trong `knowledge/published/**` phải có front matter tối thiểu:
...
schema_hash: <sha256>
source_review_path: metadata/review/commerce_demo/orders.md
...
9. Bot chỉ commit `knowledge/published/**` với message `docs(ai): regenerate published metadata`.
10. Commit bot tạo một lượt Pull Request workflow mới khi dùng GitHub App/PAT phù hợp.
11. Lượt CI mới phát hiện chỉ `knowledge/published/**` thay đổi nên không generate lại; nó chỉ validate published output và hoàn tất required check.
...
1. PR merge vào `main`.
2. Workflow **Index Knowledge** đọc diff trong `knowledge/published/**`.
...
- Generate step chỉ chạy khi `schema/raw/**`, `metadata/review/**`, `prompts/**`, `guidelines/**` hoặc contract config thay đổi.
- Bot-only commit chỉ thay đổi `knowledge/published/**`; lượt tiếp theo chỉ validate, không generate.
...
jobs:
  sync:
    ...
    steps:
      ...
      - run: metadata draft
      - run: metadata validate-review metadata/review
...
- Chỉ commit `schema/raw/**` và `metadata/review/**`.
...
```text
Input trigger generate:  schema/raw/**, metadata/review/**, prompts/**
                         guidelines/**
Bot output allowlist:    knowledge/published/**
Bot-only commit lần hai: chỉ validate, không generate
Human sửa published:     fail; reviewer phải sửa metadata/review/**
```
...
- File tương ứng trong `metadata/review/commerce_demo/`.
...
- Chỉ file trong `knowledge/published/commerce_demo/`.
...
- Bot sửa file ngoài `knowledge/published/**` trong commit generate.
...
```text
/metadata/review/commerce_demo/       @your-org/commerce-domain @your-org/data-team
/knowledge/published/commerce_demo/  @your-org/commerce-domain @your-org/data-team
...
- Commit sửa `metadata/review/**` kích hoạt Metadata PR workflow.
